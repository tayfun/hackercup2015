...
def print_case(case, success):
    print "Case #%d: %s" % (case, 'yes' if success else 'no')


input_file = open('new_years_resolution.txt', 'r')
number_of_cases = int(input_file.readline())
for case in xrange(1, number_of_cases + 1):
    p, c, f = map(lambda x: int(x), input_file.readline().split())
    number_of_foods = int(input_file.readline())
    foods = []
    for i in xrange(number_of_foods):
        fp, fc, ff = map(lambda x: int(x), input_file.readline().split())
        if fp > p or fc > c or ff > f:
            continue
        foods.append((fp, fc, ff))

    success = False
    for r in xrange(1, number_of_foods + 1):
        food_combinations = combinations(foods, r)
        for single_combination in food_combinations:
            result_tuple = reduce(lambda x, y: (x[0] + y[0],
                                                x[1] + y[1],
                                                x[2] + y[2]),
                                  single_combination)
            if result_tuple[0] == p and result_tuple[1] == c \
                    and result_tuple[2] == f:
                success = True
                break
